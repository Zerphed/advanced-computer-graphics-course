...
public:
	...
	{
		 m_scale = Mat4f::scale(Vec3f(m_size, 1.0f));
		 m_pdf = 1.0f / (4.0f * m_size.x * m_size.y);
	}
	...
	void			sample( float& pdf, Vec3f& p, int randomIdx, Sequence* s ) const;
...
protected:
	...
	Vec2f	m_size;		// physical size of the emitting
	Vec3f	m_E;		// diffuse emission (W/m^2)

	Mat4f   m_scale;	// holds the scaling matrix for this area light

	float	m_pdf;		// holds the pdf for this area light
};
