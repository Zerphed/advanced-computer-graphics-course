...
int Sampler::maxUniformSamples = 7;
Sampler::SequenceMode Sampler::sequenceMode = Sampler::SequenceMode_Random;
std::vector<float> Sequence::haltonBase2 = std::vector<float>();
std::vector<float> Sequence::haltonBase3 = std::vector<float>();
const int Sequence::maxHaltonIndex = HALTON_MAX_IDX;
...
Vec4f Renderer::computeShadingAmbientOcclusion( const RayTracer* rt, const MeshBase* mesh, const Hit& pHit, const CameraControls& cameraCtrl ) const
{
	...
	// Initialize random
	Random rand;
	unsigned int random = rand.getU32(0, HALTON_MAX_IDX - this->m_aoNumRays);
	...
	for (int i = 0; i < this->m_aoNumRays; ++i) {
		...
		switch (samplingMode) {
			...
			{
				u = Sampler::CosineSampleHemisphere(rand, random+i, this->m_aoNumRays);
				break;
			}
		...
		if ( !rt->rayCastShadow(origin, this->m_aoRayLength * u) ) {
			noHits++;
		}
	}
