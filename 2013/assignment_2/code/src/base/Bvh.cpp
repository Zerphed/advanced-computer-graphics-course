...
float Bvh::calculateCostSAH(int splitPrim, int axis, int startPrim, int endPrim) const
{
...
int Bvh::partitionPrimitivesSAH(int startPrim, int endPrim)
{
	...
	for (int i = 0; i < 3; ++i) {
		...
		for (int it = startPrim+1; it <= endPrim; ++it) {
			if ( (cost = calculateCostSAH(it, i, startPrim, endPrim)) < bestCost ) 
			{
				bestCost = cost;
				bestPos = it;
				bestAxis = i;
...
float Bvh::calculateBBoxArea(int startPrim, int endPrim) const 
{
	Vec3f max(-FLT_MAX);
	Vec3f min(FLT_MAX);
